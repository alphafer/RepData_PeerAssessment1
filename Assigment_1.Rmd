...
**ANSWER:** The mean total number of steps taken per day is **`r meanStepsPerDay`**. The histogram shows a reasonable distribution where the mean and median are close enough to consider the mean representative of the daily activity.
...
**ANSWER:** The interval **`r maxStepInterval`** is the one with a maximum averaged number of steps of **`r round(stepsInterval, 1)`**.
...
Missing <- sum(naIndex)
```
The number of missing values is **`r Missing`**. One approache will be replacing the missing values with zeros but this will squeue the data towards lower values. A better approache will be replacing the missing data with the mean value on the same time interval, across all days.
...
The mean number of steps per days, after remplacing missing data, is **`r meanStepsPerDayNoNa`** and the median number is pretty much the same  **`r medianStepsPerDayNoNa`**. this indicates that the distribution has not changed drastically and the gap between the mean and the mediam has closed.
